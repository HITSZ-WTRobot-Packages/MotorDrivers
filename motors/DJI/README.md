...
`DJI` 的反馈只直接给出单圈机械角度，因此驱动需要靠“过零检测”把它展开成连续角度。

当前实现使用 `90 / 270 deg` 阈值判断是否跨过了 `0 deg`。这套逻辑成立的前提，是相邻两帧之间不会跨过
整整一圈。

对当前已支持的两个型号，按“减速后的输出最大速度约 500 rpm、反馈频率 1 kHz”估算：
...
但要注意，`M2006_C610` 的理论单帧最大转角约 `108 deg`，已经超过当前 `90 deg` 的判定裕量。源码里已经按
要求保留了 `TODO: fixbug:` 说明：极限工况下，当前 `90 / 270 deg` 阈值仍可能漏判一次过零。
